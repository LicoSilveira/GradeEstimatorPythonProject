...
Unit6_core_assessment_points = 0 #full points for unit 6 core assessment
...
#Display total points for unit 6
...
#Unit 7 points earned
Unit7_discussion_points = 50 #full points for unit 7 discussion
Unit7_course_project_points = 50 #full points for unit 7 course project
Unit7_core_assessment_points = 50 #full points for unit 7 core assessment

#Calculate total points for unit 7
total_points_unit7 = (Unit7_discussion_points + 
                      Unit7_course_project_points + 
                      Unit7_core_assessment_points)

#Display total points for unit 7
print(f"Unit 7 Total Points: {total_points_unit7}")

#Check if maximum points were achieved for each task
print(f"Got maximum points for Unit 7 discussion? {Unit7_discussion_points == task_maximum_points}")
print(f"Got maximum points for Unit 7 course project? {Unit7_course_project_points == task_maximum_points}")
print(f"Got maximum points for Unit 7 core assessment? {Unit7_core_assessment_points == task_maximum_points}")

#Unit 8 points earned
Unit8_discussion_points = 50 #full points for unit 8 discussion
Unit8_course_project_points = 50 #full points for unit 8 course project
Unit8_core_assessment_points = 0 #full points for unit 8 core assessment

#Calculate total points for unit 8
total_points_unit8 = (Unit8_discussion_points + 
                      Unit8_course_project_points + 
                      Unit8_core_assessment_points)

#Display total points for unit 8
print(f"Unit 8 Total Points: {total_points_unit8}")

#Check if maximum points were achieved for each task
print(f"Got maximum points for Unit 8 discussion? {Unit8_discussion_points == task_maximum_points}")
print(f"Got maximum points for Unit 8 course project? {Unit8_course_project_points == task_maximum_points}")
print(f"Got maximum points for Unit 8 core assessment? {Unit8_core_assessment_points == task_maximum_points}")


#Lists to store points earned for each assignment
total_discussion_points = [Unit1_discussion_points, Unit2_discussion_points, Unit3_discussion_points, Unit4_discussion_points, Unit5_discussion_points, Unit6_discussion_points, Unit7_discussion_points, Unit8_discussion_points]
total_course_project_points = [Unit1_course_project_points, Unit2_course_project_points, Unit3_course_project_points, Unit4_course_project_points, Unit5_course_project_points, Unit6_course_project_points, Unit7_course_project_points, Unit8_course_project_points]
total_core_assessment_points = [Unit1_core_assessment_points, Unit3_core_assessment_points, Unit5_core_assessment_points, Unit7_core_assessment_points]
...
# Define base directory (adjust this to your actual directory)
BASE_DIR = r'c:\Users\licoc\OneDrive\Desktop\CIS615\GradeEstimatorCourseProject\unit7'

# Step 1: Define function to calculate grades from a CSV file using tasks.json
def calculate_grade(csv_file):
    """Calculate total points, percentage, and letter grade from a given CSV file."""
    # Check if the file exists
    if not os.path.exists(csv_file):
        print(f"Error: File '{csv_file}' not found. Please ensure it exists in the directory.")
        return None, None, None
    
    # Read the grades CSV file
    grades_df = pd.read_csv(csv_file, index_col=0)  # Load CSV with first column as index
    
    # Clean the data (replace < 0 with 0, > max_points with max_points)
    cleaned_df = grades_df.copy()  # Create a copy to preserve original data
    max_points_dict = {assignment['name']: assignment['max_points'] 
                       for assignment in assignment_rules['assignments']}  # Max points per task type
    for assignment_type in cleaned_df.index:
        max_score = max_points_dict.get(assignment_type.replace('Discussion', 'Discussions'), 50)  # Get max points
        cleaned_df.loc[assignment_type] = cleaned_df.loc[assignment_type].apply(
            lambda x: 0 if x < 0 else (max_score if x > max_score else x))  # Clean grades
    
    # Calculate total points earned
    total_points_earned = cleaned_df.sum().sum()  # Sum all grades across all weeks and assignments
    
    # Calculate maximum possible points from tasks.json (already loaded as assignment_rules)
    max_points_total = sum(task['num_tasks'] * task['max_points'] 
                           for task in assignment_rules['assignments'])  # Total max points
    
    # Display current grade and maximum grade
    print(f"You have {total_points_earned} out of {max_points_total}")  # Show total points
    
    # Calculate percentage
    percentage = (total_points_earned / max_points_total) * 100  # Convert to percentage
    print(f"Your percentage is: {percentage:.2f}")  # Display percentage with 2 decimal places
    ...
    print(f"Your letter grade is: {letter_grade}")  # Show the calculated letter grade
    
    return total_points_earned, percentage, letter_grade  # Return values for potential further use

# Step 2: Calculate and display grades for grades_0.csv
print("\nGrades based on grades_0.csv (no submission for Unit 8):")
calculate_grade(os.path.join(BASE_DIR, 'grades_0.csv'))  # Run calculation for grades_0.csv

# Step 3: Calculate and display grades for grades_50.csv
print("\nGrades based on grades_50.csv (max points for Unit 8 Discussion and Course Projects):")
calculate_grade(os.path.join(BASE_DIR, 'grades_50.csv'))  # Run calculation for grades_50.csv

# Optional Step 4: Ask user which file to use for grade calculation
csv_choice = input("\nWhich file to use for grade calculation? (grades_0.csv or grades_50.csv): ")
full_path = os.path.join(BASE_DIR, csv_choice)
print(f"\nGrades based on {csv_choice}:")
calculate_grade(full_path)  # Calculate grades based on user choice

# Optional Step 5: Allow manual input for Unit 8 grades and recalculate
print("\nEnter Unit 8 grades manually (optional):")
unit8_discussion = int(input("Enter Unit 8 Discussion grade (0-50): "))  # Get Discussion grade
unit8_project = int(input("Enter Unit 8 Course Project grade (0-50): "))  # Get Course Project grade

# Load grades_0.csv as base and modify Unit 8
grades_manual_df = pd.read_csv(os.path.join(BASE_DIR, 'grades_0.csv'), index_col=0)  # Use grades_0.csv as starting point
grades_manual_df['Week 8'] = [unit8_discussion, unit8_project, 0]  # Update Unit 8 with manual input
grades_manual_df.to_csv(os.path.join(BASE_DIR, 'grades_manual.csv'))  # Save to a new file

# Calculate and display grades based on manual input
print("\nGrades based on manual input for Unit 8:")
calculate_grade(os.path.join(BASE_DIR, 'grades_manual.csv'))  # Run calculation for manual grades
